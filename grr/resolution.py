...
class RedundancyResolution:
            ...
            if self.solver.is_continuous(
                ...
                if self.planning_mode:
                    ...
                    if self.plan_path is None:
                        self.plan_path, _ = self.plan(curr_point, target_point)
                    ...
                    # Path is already planned, follow it
                        ...
                        if self.path_index < len(self.plan_path):
                            return self.plan_path[self.path_index]
                        else:
                            self.planning_mode = False
                            self.plan_path = None
                            self.path_index = 0
    ...
    def plan(self, point1, point2, interpolation=8):
        """Plan a trajectory from point1 to point2"""
        ...
        # If no neighbor is found, do not plan
        ...
        # Add point1 to neighbor1 and point2 to neighbor2
        path_points = [self.graph.nodes[p]["point"] for p in path]
        path_points = [point1] + path_points + [point2]

        # Interpolate along the path
        num_div = interpolation
        w_path = []
        c_path = []
        for point_i, point_j in zip(path_points[:-1], path_points[1:]):
            for k in range(num_div):
                ...
                sub_waypoints = self.solve(sub_point)
                w_path.append(sub_point)
                c_path.append(sub_waypoints)
        # num_div doesn't consider the last point
        w_path.append(point2)
        c_path.append(self.solve(point2))

        # Check sanity
        # as point1 and point2 may not have valid path to the graph
        for i, q in enumerate(c_path):
            if q is None:
                print("No valid point1 point2")
                return [], []
